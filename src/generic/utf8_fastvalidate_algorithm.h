...
struct utf8_checker {
  ...
  really_inline void check_first(const simd8x64<uint8_t> in) {
    ...
    if (unlikely(bits.any_bits_set_anywhere(0x80u))) {
      this->check_utf8_bytes(in.chunks[0], uint8_t(0));
      for (int i=1;i<simd8x64<uint8_t>::NUM_CHUNKS;i++) { this->check_utf8_bytes(in.chunks[i], in.chunks[i-1]); }
      return;
    }

    // We don't have to do anything for ASCII on the first iteration.
  }

  really_inline void check_next(const simd8x64<uint8_t> in, const simd8<uint8_t> prev_bytes, const uint8_t *buf) {
    // It it's not ascii so we have to do heavy work
    simd8<uint8_t> bits = in.reduce([&](auto a, auto b) { return a | b; });
    if (unlikely(bits.any_bits_set_anywhere(0x80u))) {
      this->check_utf8_bytes(in.chunks[0], prev_bytes);
      for (int i=1;i<simd8x64<uint8_t>::NUM_CHUNKS;i++) { this->check_utf8_bytes(in.chunks[i], in.chunks[i-1]); }
      return;
    }
