...
really_inline simd8<uint8_t> get_byte_1_2_errors(const simd8<uint8_t> prev_bytes_4, const simd8<uint8_t> high_bits, const simd8<uint8_t> prev_high_bits) {
  //
  ...
  const simd8<uint8_t> byte_1_flags = (prev_bytes_4 & 0x0F).lookup_16<uint8_t>(
    // ____[00__] ________
...
really_inline simd8<uint8_t> check_utf8(const simd8<uint8_t> bytes, const simd8<uint8_t> prev_bytes, const simd8<uint8_t> prev_bytes_4) {
  ...
  const simd8<uint8_t> high_bits = bytes.shr<4>();
  const simd8<uint8_t> prev_high_bits = prev_bytes.shr<4>();
  const simd8<uint8_t> lead_flags = get_lead_flags(high_bits, prev_high_bits);
  const simd8<uint8_t> byte_1_2_errors = get_byte_1_2_errors(prev_bytes_4, high_bits, prev_high_bits);
  ...
  const simd8<uint8_t> too_large = bytes.saturating_sub(0b11111000-1); // too-large values will be nonzero
...
public:
  ...
  really_inline void check_utf8(const simd8x64<uint8_t> input, const uint8_t* buf, simd8<uint8_t> prev_input, simd8<uint8_t> prev_input_4) {
    this->error |= utf8_validation::check_utf8(input.chunks[0], prev_input, prev_input_4);
  ...
  really_inline void check_first(const simd8x64<uint8_t> input, const uint8_t* buf) {
    if (unlikely(has_utf8(input))) {
      this->check_utf8(input, buf, uint8_t(0), input.chunks[0].prev<4>());
    }
    // We don't bother checking ASCII on the first block. It's definitely OK.
  }

  really_inline void check_next(const simd8x64<uint8_t> input, const uint8_t *buf) {
    if (unlikely(has_utf8(input))) {
      this->check_utf8(input, buf, buf-sizeof(simd8<uint8_t>), buf-4);
    } else {
  ...
  really_inline void check_last(const simd8x64<uint8_t> input, const simd8<uint8_t> prev_bytes, const uint8_t *buf) {
    if (unlikely(has_utf8(input))) {
      this->check_utf8(input, buf, prev_bytes, input.chunks[0].prev<4>(prev_bytes));
      return;
    }
    // If the previous block had incomplete UTF-8 characters at the end, we now know there are no
    // more continuation bytes, so it's an error.
